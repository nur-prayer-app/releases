    ...
    function wireSettingsTab(tab) {
        ...
        if (tab === 'data') {
            ...
            $('#clear-data')?.addEventListener('click', async () => {
                ...
                ['nur-stats-range', 'nur-push-timestamps', 'nur-prayer-day-ts', 'nur-pkce-verifier', 'nur-last-sync', 'nur-sync-session', 'nur-last-sync-v2', 'nur-field-ts'].forEach(k => localStorage.removeItem(k));
                location.reload();
            });
        }
